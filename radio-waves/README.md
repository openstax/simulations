...
3. From the root `radio-waves` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `radio-waves` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `radio-waves` directory.

### Testing

To run command-line tests, run `grunt test` from the root `radio-waves` directory.
...
From the root `radio-waves` directory, run `npm run-script upgrade`, which executes the following commands:
