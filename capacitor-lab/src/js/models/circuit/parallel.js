define(function (require) {
    ...
    var ParallelCircuit = AbstractCircuit.extend({
        ...
        initWires: function(config, numberOfCapacitors) {
            ...
            this.wires.push(new WireBatteryToCapacitorsTop(attrs, opts));
            this.wires.push(new WireBatteryToCapacitorsBottom(attrs, opts));
        },
        ...
        intersectsSomeTopPlate: function(shape) {
            for (var i = 0; i < this.capacitors.length; i++) {
                if (this.capacitor.intersectsTopPlate(shape))
                    return true;
            }
            return false;
        },
        ...
        intersectsSomeBottomPlate: function(shape) {
            for (var i = 0; i < this.capacitors.length; i++) {
                if (this.capacitor.intersectsBottomPlate(shape))
                    return true;
            }
            return false;
        }
