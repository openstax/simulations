...
3. From the root `capacitor-lab` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `capacitor-lab` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `capacitor-lab` directory.

### Testing

To run command-line tests, run `grunt test` from the root `capacitor-lab` directory.
...
From the root `capacitor-lab` directory, run `npm run-script upgrade`, which executes the following commands:
