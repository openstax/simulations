...
3. From the root `electric-field-of-dreams` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `electric-field-of-dreams` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `electric-field-of-dreams` directory.

### Testing

To run command-line tests, run `grunt test` from the root `electric-field-of-dreams` directory.
...
From the root `electric-field-of-dreams` directory, run `npm run-script upgrade`, which executes the following commands:
