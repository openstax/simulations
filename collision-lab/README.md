...
3. From the root `collision-lab` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `collision-lab` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `collision-lab` directory.

### Testing

To run command-line tests, run `grunt test` from the root `collision-lab` directory.
