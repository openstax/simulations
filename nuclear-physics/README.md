...
3. From the root `nuclear-physics` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `nuclear-physics` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `nuclear-physics` directory.

### Testing

To run command-line tests, run `grunt test` from the root `nuclear-physics` directory.
...
From the root `nuclear-physics` directory, run `npm run-script upgrade`, which executes the following commands:
