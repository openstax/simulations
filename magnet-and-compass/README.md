...
3. From the root `magnet-and-compass` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `magnet-and-compass` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `magnet-and-compass` directory.

### Testing

To run command-line tests, run `grunt test` from the root `magnet-and-compass` directory.
...
From the root `magnet-and-compass` directory, run `npm run-script upgrade`, which executes the following commands:
