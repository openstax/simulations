...
This simulation is based off PhET's Energy Forms and Changes simulation, which can be [found here](http://phet.colorado.edu/en/simulation/energy-forms-and-changes).
...
3. From the root `energy-forms-and-changes` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `energy-forms-and-changes` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `energy-forms-and-changes` directory.

### Testing

To run command-line tests, run `grunt test` from the root `energy-forms-and-changes` directory.
...
From the root `energy-forms-and-changes` directory, run `npm run-script upgrade`, which executes the following commands:
