...
3. From the root `battery-resistor-circuit` directory, run `npm install` in the command line to install test and build dependencies.
...
From the root `battery-resistor-circuit` directory, run `grunt dist`.

The `dist` directory containing the built site will be added to the root `battery-resistor-circuit` directory.

### Testing

To run command-line tests, run `grunt test` from the root `battery-resistor-circuit` directory.
...
From the root `battery-resistor-circuit` directory, run `npm run-script upgrade`, which executes the following commands:
